...
**Grim is the *talk* layer; [[../04-dev-languages/13-ponytail|ponytail]] is the *build* layer.** Ponytail is an always-on Claude Code plugin that forces the laziest solution that actually works (YAGNI, stdlib-first, shortest diff). It governs *what gets built*; Grim governs *tone and reasoning*. Independent axes — a reply can be lazy in code and blunt in prose at once.
