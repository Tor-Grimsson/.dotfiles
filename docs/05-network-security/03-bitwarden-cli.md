...
- Scripts consume them as env vars (e.g. `JACKETT_API_KEY`) or fetch at runtime via `bw get password <item>` — see [[../12-scripts/07-torrent|Torrent scripts]] for a live example.
