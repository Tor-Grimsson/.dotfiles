...
## Setup
A **uv tool**, not a Brewfile line (like [[../04-dev-languages/09-llm|llm]]).
1. Install: `uv tool install kanban-tui` (see [[../04-dev-languages/04-uv|uv]]).
...
`brew bundle` already installs [[../04-dev-languages/04-uv|uv]]. Then:
```sh
