...
`bootstrap-cli.sh` gets a foreign/SSH box provisioned (see [[operations/01-dotfiles/INDEX|How this repo works]]). This is the next layer: actually working on it — nvim as the editor, `git`/`gh` for push/pull/PRs, secrets with no GUI available, and a two-GitHub-account setup for practicing the real collaboration flow.
