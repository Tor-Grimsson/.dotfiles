...
status: active
updated: 2026-07-08
description: What Claude Code skills are, how they're sourced from kol-system (ARCHITECTURE §4) vs local-authored, the whole-dir symlink mechanism, and the 36 installed skills grouped by job.
aliases:
  - skills
tags:
  - project/dotfiles
...
| **Utility** (3) | `claude-clear` (restate the last reply, tighter) · `claude-bullet` (reformat the last reply into bullets/lists/checks) · `keys-add` (maintain the `keys` keybind reference — add/fix an entry in `keys/keybinds.md` the right way) |

Count check: 12 + 3 + 3 + 3 + 4 + 8 + 3 = 36.
