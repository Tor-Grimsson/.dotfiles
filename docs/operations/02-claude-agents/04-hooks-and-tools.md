...
status: active
updated: 2026-07-08
description: The wiring around the agent layer — the statusline / doc-sync / reinforce hooks, settings.json config (permissions, plugins, voice/effort/tui), Claude Code plugins (ponytail, rust-analyzer-lsp), and MCP tools (playwright, glif) registered via bootstrap.
aliases:
  - hooks-and-tools
tags:
  - project/dotfiles
...
Hooks are shell commands the **harness** runs on lifecycle events (not the agent). Three are wired — one statusline + two behavioural, all global (dotfiles), so they apply in every repo:
...
- **`UserPromptSubmit`** → `hooks/agent-reinforce.sh` (2026-07-08) — injects report-shape + standing-rules + no-git reinforcement on a cadence (full on turn 1, compact every ~5 turns), keyed by session_id; text in `hooks/reinforce-{full,compact}.txt`. **Replaced** the old 4-skill `agent-reinforce` bundle (`agent-output-format` + `-rules` + `-memory` + the bundler); re-grounds mid-session, which the skills couldn't.
...
| `hooks` | `PostToolUse(Edit\|Write)` → doc-sync-reminder · `UserPromptSubmit` → agent-reinforce (both above) |
