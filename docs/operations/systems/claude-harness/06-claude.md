...
- **Caveats, file-lists, side-notes, provisioning status, and session-log/AGENT-CONTEXT writes fold into one footer line** — e.g. `Docs: (3) · Llm/context: (2)` — capitalised labels, counts in parentheses, ending on the last count, never trailed as prose. No "Session log created at …" restatement — that's a token (`Llm/context: (N)`), not a sentence. **Git is never mentioned at all** — `git: untouched` was a legal token until 2026-08-01 and is now banned; say nothing about the repo's state.
