...
related:
  ...
  - "[[03-agents|subagents]]"
  - "[[operations/systems/claude-harness/INDEX|Claude & Agents]]"
---
...
| **Design system / brand** (5) | `lobby-ds` — stage a component into the DS lobby as a spec · `lobby-icon` — promote a repo's icon UP into kol-icon-set: clean (currentColor, strip export junk, normalise name), check (stroke-weight, keyline, false/expanded stroke, name collision), drop into the set. The icon-SVG sibling of `lobby-ds` — emits the cleaned SVG, not a spec, since an icon needs no re-authoring · `kol-press-research` — press/mention/timeline research emitting brand-manifest entries (judgment half of the `kol-scrape` CLI) · `kol-type-conform` — enforce the KOL type protocol (JetBrains mono, the wrap/no-wrap line-height fault line) on ported or authored code · `claude-kol-ds` — mandatory orientation gate: read EVERY theme+framework CSS file (the `kol-theme.css` `@import` cascade) + foundations docs and produce a design-system overview *before* any edit/answer in kol-design-system |
...
| **Utility** (7) | `ref-add` (maintain the ref cards — add/fix an entry in `ref/*.md` / `files/folders.md` the right way; replaced keys-add + files-add 2026-07-29) · `claude-npm` (check a JS project's deps for updates — detect the PM, run `outdated`, report current→wanted→latest; read-only) · `kol-appliant` (bring a tool up to / audit it against the kol-appliant doc standard) · `kol-goal` (the "ralph loop" — a goal the `goal-loop` Stop hook won't let you stop on until `/kol-goal done`. **`- [ ]` lines in the goal file ARE the list**: `done` with any box unticked is refused, the goal flips back to `active`, and the next item is re-injected by name — added 2026-08-01 after three separate sessions closed a multi-item goal on item 1. A file with no checkboxes behaves exactly as before, and `blocked` + the iteration cap still release. **`/kol-goal-force`** additionally polices the *exits*: it reads the last assistant message and refuses a turn that ends by handing the decision back, and refuses a `blocked:` whose reason is a preference rather than an absent capability. The iteration cap and `stop_hook_active` are never policed — a loop that can't be escaped at the cap is the 2026-07-31 deadlock) · the **`lobby-*` family** (8: `lobby` router · `lobby-list` read · `lobby-dotfiles`/`-humpty`/`-web`/`-ds` writers · `lobby-icon` · `lobby-hygiene` audit — the cross-repo ticket system, spec at `docs/operations/systems/lobby/`) · `kolds` (one word = the kol-ds-ui design system: repo, 15 packages, consumers, rules) · `kolds-ref` (cite an existing token/component before proposing anything new) |

### The `tmpl-` family — output contracts (built 2026-07-30)

One namespace for "how the reply must be shaped". Printable surface: `ref-skill`.

| skill | contract |
|---|---|
| `tmpl-present` | present the updated understanding + steps to success, then STOP for approval — build nothing |
| `tmpl-ask` | four lines: ask · state · blocker · need. Never "shall I proceed?" |
| `tmpl-human` | write for a tired human — ≤3-line paragraphs, one idea per line, tables for parallel facts |
| `tmpl-uncanny` | stop performing human — no apology, validation, sympathy, or enthusiasm |
| `tmpl-hl` | high-level shape only, then walk one piece at a time |
| `tmpl-wl-100 / -80 / -60 / -40 / -10` | a **standing** body budget (~250 / 200 / 150 / 100 / 25 words) that does NOT decay; overflow folds into the footer. Anti-bypass clauses: no reply-splitting, no "one more thing", no overflow in code blocks |
| `tmpl-stfu` | family name for the humpty `stfu` clamp (plugin-owned; this is the local alias) |
| `tmpl-bullet` · `tmpl-clear` | renamed from `claude-bullet` / `claude-clear`; the old names still answer |
| `tmpl-proposal` | stage a visual side-by-side for approval (pre-existing) |
| `dump` | research printed unparsed with the verdict buried — re-emit verdict-first |

**Modes** (what the agent may *do* this turn, as opposed to how it writes): `jana`/`j`/`yana` = answer only, zero tools · `rosa`/`r` = research with tools, change nothing · `kol-goal` = can't stop until done.

Ported to `kol-dumpty/humpty/lobby/tmpl-family.md` — the muzzle repo owns the question of whether `tmpl-wl-*` becomes a hook-enforced budget rather than an honour system.
