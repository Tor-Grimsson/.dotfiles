...
## The skeleton — v2 (current, 2026-08-01)
...
**One-line bold sub-title**

<a TABLE only if there are ≥2 rows of the same kind>
<ONE record → an aligned Label: block instead, one line per label, never wrapped>
<at most ONE line of follow-up after a block>

Docs: (3) · Llm/context: (2) · Caveats: (1)
````
...
| Rule | v1 (parked) | v2 |
|---|---|---|
| **Git** | `git: untouched` was a legal footer token | **Banned outright.** Never in prose, never as a token, never at all |
| **Footer counts** | `docs: 3 · caveats: 1` | **`Docs: (3) · Caveats: (1)`** — capitalised label, count in parens |
| **Case** | drifted to lowercase cells and labels | **Sentence case everywhere** — cells, labels, list items |
| **One record** | rendered as a one-row table | **Aligned `Label:` block** — a one-row table burns a full box border and wraps every cell |
| **Sub-titles** | prose sentence introducing a block | **ONE bold line**, nothing after it |
| **Follow-up** | a paragraph under a block | **ONE line max** — more belongs in the doc |
| **Closing phrase** | ended `say "show noise" to expand` | cut 2026-08-01 — the line ends on its last count |

The one-record rule, shown:
...
> **If a value doesn't fit one line, cut the value — never wrap it.** Paths, commands and counts are protected; the explanation wrapped around them is not. A wrapped cell in a box table is the single biggest source of the noise this version exists to remove.

## The skeleton — v1 (PARKED 2026-08-01)

Kept for the record; **superseded by v2 above**. Do not write replies to this shape.

````text
footer: docs: N · llm/context: N · caveats: N · git: untouched · say "show noise" to expand
````

v1 footer tokens were terse counts + status, `·`-separated, lowercase, and the line **always ended with `say "show noise" to expand`**. Both the git token and the closing phrase are now defects.
