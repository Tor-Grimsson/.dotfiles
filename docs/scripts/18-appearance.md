...
`raycast/scripts/` — add the directory once via Raycast → Settings → Extensions → Scripts → **Add Directories**, then record hotkeys per-command (same setup as [[05-aerospace|AeroSpace's enable/disable pair]]).
...
- `docs/operations/systems/terminality/kol-dash/` — the wider wake-alarm + dashboard system this is one piece of
