...
| _(none)_ | [[scripts/ref-system/INDEX|Reference cards]] | 1 | `ref [card] <word>` / `ref-<card>` — the reference-card system: what a TOOL does, glow-rendered tables narrowed by section-title word. **`ref --cards` is the live list** — this row deliberately does not copy it. Own folder: [[scripts/ref-system/INDEX|ref-system]] |
| _(none)_ | [[notes-system\|notes]] | 1 | `notes-<card> <word>` — explainer cards: what a thing MEANS. Sibling of `ref`, separate because ref's 46-char cell lint is right for keybinds and wrong for prose. The directory is the registry |
| _(none)_ | [[md-preview\|md-preview]] | 1 | `md-preview` — yazi's markdown previewer: three modes on `prefix v` (`full` keeps the frontmatter both renderers discard), rendered to the pane width piper reports |
