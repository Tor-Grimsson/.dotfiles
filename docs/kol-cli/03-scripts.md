...
Everything in `~/.dotfiles/bin` (symlinked to `~/bin`, on `PATH`). **Every script answers `-h` / `--help`** with purpose, args, and examples — so this is just the map; run `<script> -h` for the detail. Grouped by job below; the full per-family write-ups live in [[scripts/INDEX|docs/scripts]].
...
## Appearance & wake
Theme control + a real clock-time wake-up bundle. Full doc: [[18-appearance|Appearance & wake automation]].
...
- This is the quick map — the authoritative, per-script detail (flags, gotchas, worked examples) is in [[scripts/INDEX|docs/scripts]], one doc per family.
