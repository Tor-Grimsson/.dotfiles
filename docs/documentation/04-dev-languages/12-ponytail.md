...
- **Statusline badge — retired 2026-07-28:** the dotfiles `claude/hooks/statusline.sh` badge slot now shows the **humpty dial** (solid tmux-yellow `humpty-dumpty` block, level suffix off-default) reading `.humpty-active`; the ponytail badge + its `.ponytail-active` flag are gone. Ponytail itself is installed-but-disabled (`/plugin` re-enables it).
