...
- Daily driving is keybound (`ref-desk toggles`): **cmd-alt-u** open/close (`bin/ubersicht-toggle`) · **cmd-alt-r** refresh, double-pass because simple-bar imports `~/.simplebarrc` one refresh behind rendering it (`bin/ubersicht-refresh`).
...
- Debug: Übersicht menubar icon → *Open Debug Console* (a full web inspector — `console.log` works). **Icon-only, and unscriptable** — `Uebersicht.sdef` exposes exactly three commands (`refresh`, `reload`, `quit`), so unlike refresh this can never be keybound.
- **Which screen the widgets draw on — `bin/ubersicht-screen`** (added 2026-07-31). Widgets default to `showOnAllScreens`, but the snap gutter that reserves room for them is **per-monitor** in `aerospace.toml` (`outer.right = [{ monitor.main = 304 }, 10]`). Widgets on every screen + a gutter on one = tiled windows sliding under the widget column on the other. `ubersicht-screen main` pins them to the main display so the two halves agree; `ubersicht-screen` alone prints the current state; `all` reverts. Scriptable because `Uebersicht.sdef` exposes `showOnMainScreen` / `showOnAllScreens` as `access="rw"` — **not** a GUI-only setting, as was briefly claimed here. macOS "main" = the display carrying the menu bar; on this desk that's the iMac's built-in Retina 5K, not the U32J59x. Übersicht stores it, so **re-run after a reinstall or a wiped WebKit store**. Card: `ref-desk widgets`.
- **Can't find the icon?** Übersicht is an agent app (`LSUIElement = 1`) — no Dock icon, no window, it never "opens"; the menu-bar item is its entire UI. Ours registers at `NSStatusItem Preferred Position Item-0 = 830`, far left in the strip, which puts it inside **Hidden Bar**'s collapsed section. Click Hidden Bar's `›` to reveal, or cmd-drag the icon right of the `|` separator to pin it.
- **Browser view: `http://127.0.0.1:41416` — not `localhost`.** Same page the WebView renders, with full Safari devtools. The server binds `127.0.0.1` and rejects the `localhost` Host header with a bare **400** on every path (verified 2026-07-31), which reads as "the server is broken" when it is just the hostname.
...
- **Nothing renders** → `cmd-alt-r` (`bin/ubersicht-refresh`) — no icon needed; then Debug Console for the actual error.
- **simple-bar shows "loading"** → its widgets folder name must be exactly `simple-bar`; check `windowManager: "aerospace"` in `~/.simplebarrc`.
- **simple-bar shows "yabai is not running"** (hit 2026-07-27) → the localStorage store is gone entirely (`~/Library/WebKit/tracesOf.Uebersicht/WebsiteData/LocalStorage` absent), so the bar boots on stock defaults — and stock `windowManager` is yabai. Not a config problem: `~/.simplebarrc` is fine, the import just hasn't landed. Fix: `cmd-alt-r` (`bin/ubersicht-refresh`, double-pass) — the store re-seeds from the file on the first pass and renders on the second.
...
- **Widget cards look solid, not glassy** → by design: the WebView doesn't run backdrop-blur, so real translucency double-exposes the desktop behind. The kol widgets use near-solid `rgba(bg, 0.96)` instead (moved here from the widgets ref card, 2026-07-29).
