...
status: active
updated: 2026-07-08
description: SQLite-backed shell history with a full-screen fuzzy search UI — records exit code/duration/cwd/host per command, scoped search (global/host/session/directory), optional encrypted multi-machine sync. Replaces fzf's Ctrl-R.
aliases:
  - atuin
tags:
  - domain/shell
...
covers:
  - the Ctrl-R / Up-arrow handoff from fzf
...
3. Init wired in `shell/.zshrc`, sourced **after** `source <(fzf --zsh)` so it wins the Ctrl-R bind (fzf keeps Ctrl-T/Alt-C — see [[12-fzf|fzf]])
...
| Key | Does |
|---|---|
| `Ctrl-r` | Open search (global scope); press again while searching to cycle scope |
| `Up` | Search scoped to the current directory (`filter_mode_shell_up_key_binding`) |
...
| `filter_mode_shell_up_key_binding` | `"directory"` | Up-arrow stays scoped to cwd even while Ctrl-R searches globally |
