...
#   Ctrl-a C-p   clip capture menu (fzf)  Ctrl-a C-d   layout → window (fzf)
#   Ctrl-a C-f   ref-card picker (fzf)    Ctrl-a C-l   ask llm (fzf)
...
- **Mark a pane** — `prefix m` tints the current pane so it stands out (e.g. the one LOCAL pane among SSH panes); `prefix M` clears it. (A visual tint only — not tmux's `select-pane -m` mark.)
- **Join / break panes** — `prefix <` pulls a picked window in as a pane here, `prefix >` sends the current pane into a picked window (`choose-window` → `join-pane`; join has no stock key); stock `prefix !` breaks a pane back out into its own window.
- **Popups** — floating windows over your panes (`display-popup`, tmux 3.2+), each opening in the current folder and vanishing on exit: `prefix C-t` scratch shell, `prefix C-y` [[02-yazi|yazi]] file manager, `prefix C-g` [[03-lazygit|lazygit]] git TUI, `prefix C-s` [[17-sesh|sesh]] session picker, `prefix C-p` [[08-system|clip-drop]] (clipboard-image capture menu: file/drop/note/review → `~/_inbox`), `prefix C-f` [[scripts/ref-system/01-system|ref-pick]] (ref-card drill-down: card → section → read), `prefix C-l` [[scripts/llm-pick|llm-pick]] (ask Claude: ask / continue / chat / pipe the clipboard / model override). Don't launch a popup from inside a popup — nested popups misbehave.
...
- **Agent grant** — `prefix g` toggles a 15-minute window where the Claude agent may run **read-only git + downloads** ([[11-grant|agent-grant]]: `bin/agent-grant` + the `git-gate.sh` PreToolUse hook; yellow `[GRANT git Nm]` badge in the Claude statusline while open). Press again to revoke early; expires by itself.
