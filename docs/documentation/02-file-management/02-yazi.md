...
## Future use
Per-filetype openers (e.g. route `.psd` through an [[img-from-psd|img-from-psd]] action); a `git` plugin for status flags in the list; more `ya pkg` plugins (chmod, mount, archive browsing) as the need shows up. Add them via `ya pkg add` — they vendor into the repo automatically.
