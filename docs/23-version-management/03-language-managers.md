...
related:
  ...
  - "[[../04-dev-languages/01-node|Node.js]]"
---
...
| **Python** | **uv** — Rust, also does deps + venvs | `pyenv` (version-only, the classic), `conda`/`miniconda` (data-science, ships its own Python) | `.python-version` | uv is already installed here — [[../04-dev-languages/04-uv\|uv doc]]. It manages *interpreters* too, so no separate pyenv needed. |
...
A **version manager** picks the *runtime* (which `node`). A **package manager** installs a project's *dependencies* (pnpm, npm, pip, cargo, gem). Different jobs, different layer — see the layers picture in [[../21-dotfiles/INDEX|the dotfiles model]] and the per-tool docs for [[../04-dev-languages/05-pnpm|pnpm]] and [[../04-dev-languages/04-uv|uv]]. uv blurs the line by doing both for Python; fnm is strictly a runtime switcher and leaves dependencies to pnpm/npm.
