...
related:
  ...
  - "[[../01-shell-terminal/INDEX|Shell & Terminal]]"
---
...
In practice most day-to-day config goes in `.zshrc`/`.bashrc` because you want it in every shell. This repo's [[../21-dotfiles/01-repo-model|dotfiles]] track both: `shell/.zshrc` (interactive) and `shell/.zprofile` (login, which runs `brew shellenv` arch-correctly for both Macs).
...
No hardcoded paths in that line — `fnm` is found via PATH — so it satisfies the repo's cross-arch rule ([[../21-dotfiles/01-repo-model|no hardcoded brew prefixes]]) and works on both the Intel and Apple-Silicon machines from the one shared `.zshrc`.
