...
related:
  ...
  - "[[../12-scripts/18-appearance|Appearance & wake automation]]"
...
**Scripts themselves are not re-documented here** — `os-mode.sh` / `theme-alarm.sh` already have a full per-flag reference at [[../12-scripts/18-appearance|12-scripts/18-appearance]], the established one-doc-per-family catalog every other `bin/` script uses. Duplicating that here would just be two places to keep in sync; this category links to it instead.
