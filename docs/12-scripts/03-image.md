...
- **Deps/gotchas:** imagemagick; **Ghostscript (`gs`) for PDF/EPS input**. The generic sibling of [[img-from-psd|img-from-psd]] (PSD-only) — same flag style, any source, plus a default 2000px fit and a jpg/png `-P` prompt. `-c` uses no dithering deliberately — dithering error-diffuses past the requested count and can blow past PNG's 256-entry palette cap, silently falling back to truecolor and losing the size win. `-e` is the same cover+crop mechanism [[img-canvas|img-canvas.sh]] uses for its aspect presets, exposed as a flag here instead of a separate tool — reach for `img-canvas.sh` instead when you want its aspect **presets** (9:16, 4:5, etc.) or the `-s` resolution multiplier; reach for `img-convert.sh -e` when you already know the exact `WxH` and want to keep using `-c`/`-q`/`-d`/`-a`. Full Quick-Action wiring + the `-c`/`-e` deep-dives live in [[img-convert|the deep-dive]].
