---
name: tmpl-bullet
description: User signal to reformat the last reply (or supplied text) into clean, scannable structure — bullets, numbered lists, checkboxes, tables. Structure over prose; keep the substance. Triggered by /tmpl-bullet (user-invoked only).
---
...
Family: the `tmpl-` output-contract prefix. `/claude-bullet` remains as a legacy alias for muscle memory; both invoke the same contract.
