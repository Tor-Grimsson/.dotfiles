...
- Framework packages at `~/.dotfiles/claude/packages/kol-docs-{fm,md,lib}/`
...
- **`replace`** — overwrite the target file with the source content. For files that should match exactly: protocol READMEs (e.g. `session-bridge/README.md`), framework files.
- **`notify-only`** — show the diff but don't change anything. The user decides whether to merge by hand. For files that are scaffold-seeded but expected to grow with project-specific content: `llm-context/README.md`.
...
2. **Sanity check.** Confirm the target repo has been init-agent-context'd:
   - `LLM_RULES.md` exists at root (a symlink to the generic boot file — not synced here), AND
...
3. **Enumerate source files.** Run two finds:
   ...
   - `find ~/.dotfiles/claude/packages/kol-docs-fm ~/.dotfiles/claude/packages/kol-docs-md ~/.dotfiles/claude/packages/kol-docs-lib -type f -name '*.md'` for framework files. They target `.kol/docs-framework/<package>/<relative>` in the consumer repo.
...
4. **For each source file, compute the diff state.** Build a report with one row per file:
   ...
   Read frontmatter via Read or a small Bash one-liner. Don't trust path inference — read `_template.path` from the source's frontmatter for the canonical relative path. For framework files (no `_template.path`), derive the target path as `.kol/docs-framework/<package>/<relative>` (e.g. `kol-docs-fm/01-frontmatter.md`).
...
5. **Surface the report to the user.** Print a table:
   ...
   .kol/docs-framework/kol-docs-fm/01-frontmatter.md 2    1    replace       will replace
   .kol/llm-context/README.md                    1    1    notify-only   in sync
...
- **`LLM_RULES.md` and the `/init-agent` + `/log-work` skills are out of sync scope.** `LLM_RULES.md` is a symlink to the one generic boot file (`~/.dotfiles/claude/packages/LLM_RULES.md`) — always current by definition. The skills are global (`~/.claude/skills`, dotfiles-backed), never per-repo. Neither is in the template set, so neither is synced here.
