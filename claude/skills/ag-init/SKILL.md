...
7. **Reinforcement is automatic** — the global `agent-reinforce` UserPromptSubmit hook (dotfiles `claude/hooks/agent-reinforce.sh`) injects report-shape + standing-rules + no-git reinforcement on a cadence (full on turn 1, compact every ~5 turns). Nothing to load here; it re-grounds mid-session, which the old skill bundle couldn't.
