...
**This skill is self-contained.** The full rules are below (the "syntax"). Copy-from reference shapes live in `claude/packages/kol-docs-framework/_example/` (a fully-applied fictional vault, one of every type) and Obsidian Templater files in `.../_templates/` — backup/reference, not required to act.
...
5. For exact starting shapes, copy the nearest type from `claude/packages/kol-docs-framework/_example/` — but the rules above are sufficient to author/normalise without it.
