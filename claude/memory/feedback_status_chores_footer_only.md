...
**How to apply:** Before sending, scan the last body paragraph: if it mentions git/deploy/restart or reads as a caveat, delete it or reduce it to a footer token (`flags: 2`). Findings that need his RULING go in the body as findings; chores and FYIs never do.

**Escalated 2026-07-29:** "DONT SAY THE WORD GIT unless asked." A "git round — yours" checklist item in a close-out message triggered it. The word is now retired entirely — not in body, not as a footer token — unless the user raises the topic first. He suspected a permissions change; the answer was agent sloppiness, which is worse.
