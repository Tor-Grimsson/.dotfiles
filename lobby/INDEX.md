...
|---|---|---|---|---|
| 🔵 | [humpty-gates-misfire-on-docs-and-command-text](inbox/humpty-gates-misfire-on-docs-and-command-text.md) | two **humpty** gates deny correct work, both hit live on the MBP. The **token gate** counts `--kd-*` tokens in the *session's* repo and applies the law to a file in a **different** repo — the 14 it names are dotfiles', the write was into studio16, which has none; it also fires on markdown, where literal values are the content. The **scan gate** matches the vendor-directory name anywhere in a command, so a grep that *excludes* it is denied for naming it — and writing this very ticket was denied because its prose mentions it. Not fixable here: `~/dev/projects/kol-dumpty/` is not on this machine. **Defect 3 added 2026-08-04** — the *delete* gate has the same command-text disease: a `command -v` PATH lookup plus a `grep` of a keymap was denied with the full NOTHING IS DELETED law, matched purely on tool names. The fault is a pattern across two gates, not one hook's bug | 2026-08-03 | `filed` — from a dotfiles session, for the iMac agent |
...
| 2026-08-03 | **`humpty-gates-misfire-on-docs-and-command-text` filed from the MBP — the first ticket filed *inward* because its destination repo is absent.** humpty is not checked out on this machine, so `/lobby-humpty` had nowhere to write and the brief lands in this queue to travel with the repo. Both defects were found in live use, not by reading hook source: the token gate resolving its token set from the session cwd instead of the write target's repo, and the scan gate matching a vendor-directory name anywhere in a command — including in the **prose of the file being written**, which denied this ticket twice before it could be saved. One correction inside it: the token count is real and correctly measured, just measured against the wrong tree — the first diagnosis called it fabricated |
| 2026-08-04 | **A third defect appended to `humpty-gates-misfire-on-docs-and-command-text` — the command-text fault is a pattern, not one hook.** The *delete* gate denied a purely read-only command: `command -v` PATH lookups for recovery tools plus a `grep` of `yazi/keymap.toml`, refused with the full NOTHING IS DELETED law and its `mv` instruction. No `rm`, no `mv`, no target path, no write — the only match available was the **names of the tools being looked up**. Filed as its own defect rather than folded into defect 2 because it is a **different gate** with the same disease, which changes the fix from one regex to a rule: judge by verb and target, never by substring. Notable consequence — an agent structurally cannot check whether the macOS bin-recovery CLI is installed, so it cannot document it, which is the task that hit this. **State unchanged at 🔵** — that is the user's call |
